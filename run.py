...
# Fetch credentials and streamer list from environment variables
TWITCH_USERNAME = os.environ.get("TWITCH_USERNAME", "")
TWITCH_PASSWORD = os.environ.get("TWITCH_PASSWORD", "")
STREAMER_LIST = os.environ.get("STREAMER_LIST", "")
# STREAMER_LIST should be a comma-separated string: "streamer_01,streamer_02,streamer_03"
STREAMER_NAMES = [s.strip() for s in STREAMER_LIST.split(",") if s.strip()]

if not TWITCH_USERNAME or not TWITCH_PASSWORD or not STREAMER_NAMES:
    raise ValueError(
        "TWITCH_USERNAME, TWITCH_PASSWORD, and STREAMER_LIST environment variables must be set. "
        "Example for STREAMER_LIST: streamer_01,streamer_02,streamer_03"
    )
...
twitch_miner = TwitchChannelPointsMiner(
    ...
    logger_settings=LoggerSettings(
        ...
        time_zone="Europe/Rome",                # Set a specific time zone for console and file loggers. Use tz database names. Example: "America/Denver"
...
twitch_miner.mine(
    STREAMER_NAMES,                             # Array of streamers (order = priority)
